...
$(function() {
  ...
  document.onkeyup = function(e) {
    ...
    if (e.keyCode == 27) { // Escape key
      ...
      $.post('http://pw_lockpick/escape', JSON.stringify({}));
    }
  };
...
function damagePin() {
  if (!pinLastDamaged || Date.now() - pinLastDamaged > pinDamageInterval) {
    ...
    if (pinHealth <= 0) {
      $.post('http://pw_lockpick/removepin', JSON.stringify({}));
      breakPin();
    }
  }
}
...
function outOfPins() {
  ...
  reset()
  setTimeout(function(){ location.reload(); }, 500);
  $.post('http://pw_lockpick/process', JSON.stringify({
    state: false
  }));
}
