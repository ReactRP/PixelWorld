...
function setNuiActive(boolean = true) {
  if (boolean !== isNuiActive) {
    if (boolean) {
      window.emitNet('pw_mysql:request-data');
    }
    isNuiActive = boolean;
...
window.onNet('pw_mysql:update-resource-data', (resourceData) => {
...
window.onNet('pw_mysql:update-time-data', (timeData) => {
  let timeArray = [];
...
window.onNet('pw_mysql:update-slow-queries', (slowQueryData) => {
