...
function MySQL.Sync.execute(query, params)
    ...
    exports['pw_mysql']:mysql_execute(query, safeParameters(params), function (result)
        res = result
        finishedQuery = true
    end)
...
function MySQL.Sync.fetchAll(query, params)
    ...
    local res = {}
    local finishedQuery = false
    exports['pw_mysql']:mysql_fetch_all(query, safeParameters(params), function (result)
        res = result
        finishedQuery = true
    end)
    repeat Citizen.Wait(0) until finishedQuery == true
...
function MySQL.Sync.fetchScalar(query, params)
    ...
    local res = ''
    local finishedQuery = false
    exports['pw_mysql']:mysql_fetch_scalar(query, safeParameters(params), function (result)
        res = result
        finishedQuery = true
    end)
    repeat Citizen.Wait(0) until finishedQuery == true
...
function MySQL.Sync.insert(query, params)
    ...
    local res = 0
    local finishedQuery = false
    exports['pw_mysql']:mysql_insert(query, safeParameters(params), function (result)
        res = result
        finishedQuery = true
    end)
    repeat Citizen.Wait(0) until finishedQuery == true
...
function MySQL.Sync.transaction(querys, params)
    local res = 0
    local finishedQuery = false
    exports['pw_mysql']:mysql_transaction(query, params, function (result)
        res = result
        finishedQuery = true
    end)
    repeat Citizen.Wait(0) until finishedQuery == true
...
function MySQL.Async.execute(query, params, func)
    ...
    exports['pw_mysql']:mysql_execute(query, safeParameters(params), func)
end
...
function MySQL.Async.fetchAll(query, params, func)
    ...
    exports['pw_mysql']:mysql_fetch_all(query, safeParameters(params), func)
end
...
function MySQL.Async.fetchScalar(query, params, func)
    ...
    exports['pw_mysql']:mysql_fetch_scalar(query, safeParameters(params), func)
end
...
function MySQL.Async.insert(query, params, func)
    ...
    exports['pw_mysql']:mysql_insert(query, safeParameters(params), func)
end
...
function MySQL.Async.transaction(querys, params, func)
    return exports['pw_mysql']:mysql_transaction(querys, params, func)
end

function MySQL.ready (callback)
    Citizen.CreateThread(function ()
        -- add some more error handling
        while GetResourceState('pw_mysql') ~= 'started' do
            Citizen.Wait(0)
        end
        while not exports['pw_mysql']:is_ready() do
            Citizen.Wait(0)
        end
        callback()
    end)
