...
function createOfflineAccount(cid)
    ...
    if cid then
        ...
        self.savings = MySQL.Sync.fetchAll("SELECT * FROM `banking` WHERE `type` = 'Savings' AND `cid` = @cid", {['@cid'] = self.cid})[1] or nil
        ...
        if self.query ~= nil then
            ...
            if isOnline ~= false and isOnline > 0 then
                ...
                if rTable:Bank().getBalance() < 0 and (_bankInfo.overdraft ~= 0 and rTable:Bank().getBalance() >= (_bankInfo.overdraft * -1)) then
                    -- Arranged Overdraft 1.2% apr
                    local currentOverDrawn = math.floor((math.abs(rTable:Bank().getBalance()) * 0.012))
                    if currentOverDrawn > 0 then
                        rTable:Bank().forceRemoveMoney(tonumber(currentOverDrawn), "Authorised Overdraft 1.2% ($"..currentOverDrawn..") Interest", function(done)
                        end)
                    end
                elseif rTable:Bank().getBalance() < (_bankInfo.overdraft * -1) then
                    -- Unarranged Overdraft 3.5% apr -- Over there overdraft limit, this could only really happen on force removal of funds due to interest
                    local currentOverDrawn = (math.abs(rTable:Bank().getBalance()) * 0.035)
                    if currentOverDrawn >= 1 then
                        rTable:Bank().forceRemoveMoney(math.floor(tonumber(currentOverDrawn)), "Unauthorised Overdraft 3.5% Interest", function(done)
                        end)
                    end
                end
            end
        ...
        rTable.Savings = function()
            ...
            savings.adjustStatement = function(action, amount, desc)
                ...
                if action == "deposit" then
                    MySQL.Sync.insert("INSERT INTO `bank_statements` (`account`,`character_id`,`account_number`,`sort_code`,`deposited`,`balance`,`date`,`message`) VALUES (@account, @cid, @accountnumber, @sortcode, @deposited, @balance, @date, @message)", {
                        ['@account'] = "savings",
                        ['@cid'] = self.cid,
                        ['@accountnumber'] = self.savings.account_number,
                        ['@sortcode'] = self.savings.sort_code,
                        ['@deposited'] = amount,
                        ['@balance'] = self.savingsy.balance,
                        ['@date'] = time,
                        ['@message'] = desc
                    })
                else
                ...
                if self.savings ~= nil then
                    if m and type(m) == "number" then
                        MySQL.Async.execute("UPDATE `banking` SET `balance` = `balance` + @balance WHERE `cid` = @cid AND `type` = 'Savings'", {['@balance'] = m, ['@cid'] = self.cid}, function(processed)
                            if processed > 0 then
                                self.savings.balance = (self.savings.balance + m)
                                -- Insert Banking Deposit Query for Statements
                                rTable.Savings().adjustStatement("deposit", m, desc)
                                if cb then
                                    cb(true)
                                end
                            else
                                self.savings.balance = self.savings.balance
                                if cb then
                                    cb(false)
                                end
                            end
                    ...
                    if m and type(m) == "number" then
                        MySQL.Async.execute("UPDATE `banking` SET `balance` = `balance` - @balance WHERE `cid` = @cid AND `type` = 'Savings'", {['@balance'] = m, ['@cid'] = self.cid}, function(processed)
                            if processed > 0 then
                                ...
                                rTable.Savings().adjustStatement("withdraw", m, desc)
                                if cb then
                                    cb(true)
                                end
                            else
        ...
        rTable.Bank = function()
            bank = {}
            bank.adjustStatement = function(action, amount, desc)
                local time = os.date("%Y-%m-%d %H:%M:%S")
                if action == "deposit" then
                    MySQL.Sync.insert("INSERT INTO `bank_statements` (`account`,`character_id`,`account_number`,`sort_code`,`deposited`,`balance`,`date`,`message`) VALUES (@account, @cid, @accountnumber, @sortcode, @deposited, @balance, @date, @message)", {
                        ['@account'] = "personal",
            ...
            bank.addMoney = function(m, desc, cb)
                if m and type(m) == "number" then
                        ...
                        if currentBalance ~= nil then
                            ...
                            MySQL.Async.execute("UPDATE `banking` SET `balance` = `balance` + @balance WHERE `cid` = @cid AND `type` = 'Personal'", {['@balance'] = m, ['@cid'] = self.cid}, function(processed)
                                if processed > 0 then
                                    ...
                                    -- Insert Banking Deposit Query for Statements
                                    rTable:Bank().adjustStatement("deposit", m, desc)
                                    if cb then
                                        cb(true)
                                    end
                                else
            ...
            bank.removeMoney = function(m, desc, cb)
                if m and type(m) == "number" then
                    MySQL.Async.fetchScalar("SELECT `balance` FROM `banking` WHERE `cid` = @cid AND `type` = 'Personal'", {['@cid'] = self.cid}, function(currentBalance)
                        if currentBalance ~= nil then
                            self.query.balance = currentBalance
                            ...
                            if (self.query.balance - m) >= (accountMeta.overdraft and (accountMeta.overdraft * -1) or 0) then
                                MySQL.Async.execute("UPDATE `banking` SET `balance` = `balance` - @balance WHERE `cid` = @cid AND `type` = 'Personal'", {['@balance'] = m, ['@cid'] = self.cid}, function(processed)
                                    if processed > 0 then
                                        self.query.balance = (self.query.balance - m)
                                        -- Insert Banking Withdraw Query for Statements
                                        rTable:Bank().adjustStatement("withdraw", m, desc)
                                        if cb then
                                            cb(true)
                                        end
                                    else
            ...
            bank.forceRemoveMoney = function(m, desc, cb)
                if m and type(m) == "number" then
                    MySQL.Async.fetchScalar("SELECT `balance` FROM `banking` WHERE `cid` = @cid AND `type` = 'Personal'", {['@cid'] = self.cid}, function(currentBalance)
                        if currentBalance ~= nil then
                            self.query.balance = currentBalance
