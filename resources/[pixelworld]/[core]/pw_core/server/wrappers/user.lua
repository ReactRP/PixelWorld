        ...
        if self.query[1] ~= nil then
            ...
            rTable.createCharacter = function(data)
                if data then
                    ...
                    local cid
                    local generatedEmail = data.firstName..data.lastName.."@SynCity.com"
                    ...
                    if emailCheck ~= nil then
                        repeat
                            math.randomseed(os.time())
                            generatedEmail = data.firstName..'.'..data.lastName..''..math.random(1,99)..'@SynCity.com'
            ...
            rTable.doLogin = function(cb)
                for k, v in pairs(Config.DeveloperSteams) do
                    if v == self.steam then
            ...
            rTable.saveUser = function(notify)
                ...
                if saved > 0 then
                    if notify then
                        if not temp then
                            print(' ^1[SynCity Core] ^7- User (^4'..name..'^7) has been ^2successfully ^7saved and unloaded.^7')
                        end
                    end
                    return true
                else
                    if notify then
                        if not temp then
                            print(' ^1[SynCity Core] ^7- User (^4'..name..'^7) has ^1not ^7been successfully saved but ^2successfully ^7unloaded.^7')
                        end
                    end
                    return false
                end
            ...
            if not temp then
                print(' ^1[SynCity Core] ^7- User (^4'..self.query[1].name..'^7) has been successfully loaded.^7')
            end
            return rTable
        else
            DropPlayer(src, "We could not locate your user account, please try reconnecting to SynCity again to resolve this.")
            return nil
        end
    end
end