...
AddEventHandler('pw:characterLoaded', function(unload, ready, data)
    if not unload then
        if ready then
			...
			CharacterSpawnedNow()
			StartMainThreadRunningNowPlease()
        else
            playerData = data
        end
    else
...
function CharacterSpawnedNow()
	if (not spawned) then 
		-- Try and get the saved UI data
		local uiData = GetResourceKvpString( "pw_platereader_ui_data" )
...
function PLY:VehicleStateValid()
	return characterLoaded and (playerData.job.name == 'police') and DoesEntityExist(self.veh) and self.veh > 0 and self.inDriverSeat and self.vehClassValid
end 
...
function RADAR:LoadOMData()
	...
	local rawData = GetResourceKvpString( "pw_platereader_om_data" )
...
RegisterNUICallback( "setAntennaMode", function( data ) 
	...
	if ( RADAR:IsPowerOn() and not RADAR:IsPoweringUp() ) then 
		...
		if ( RADAR:IsMenuOpen() ) then 
			...
			SetResourceKvp( "pw_platereader_om_data", omData )
		else
...
RegisterNUICallback( "saveUiData", function( data, cb )
	SetResourceKvp( "pw_platereader_ui_data", json.encode( data ) )
end )
...
function StartMainThreadRunningNowPlease()
-- Main thread
	Citizen.CreateThread(function()
...
RegisterCommand( "reset_radar_data", function()
	DeleteResourceKvp( "pw_platereader_ui_data" )
	DeleteResourceKvp( "pw_platereader_om_data" )
end, false )