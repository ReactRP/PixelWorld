...
RegisterNetEvent('pw_weed:UseSeed')
RegisterNetEvent('pw_weed:UseItem')
RegisterNetEvent('pw_weed:UseBag')
...
function MFD:Start()
    ...
    PW.TriggerServerCallback('pw_weed:GetLoginData', function(plants)
                ...
                exports['pw_progbar']:Progress(
                    {
                        ...
                        if not status then
                            ...
                            TriggerEvent('pw_weed:client:showing', false)
                            MFD:processNUI()
                        end
                    end)
            end)
...
function MFD:DrawCurText()
    ...
    local dist = #(GLOBAL_COORDS - vector3(closest.Position.x, closest.Position.y, closest.Position.z))
...
function MFD:TextHandler()
    ...
    TriggerEvent('pw_weed:client:showing', true)
    SendNUIMessage({
        action = "showPlant",
        info = closest,
...
function MFD:Awake(...)
    ...
    PW.TriggerServerCallback('pw_weed:GetStartData', function(retVal)
        self.dS = true;
        self.cS = retVal;
    end)
...
function MFD:UseSeed(seed, seedName)
...
function MFD:Sync(plant, delete)
    TriggerServerEvent('pw_weed:SyncPlant', plant, delete)
end
...
AddEventHandler('pw_weed:UseSeed', function(seed, seedName)
    if MFD.iR then MFD:UseSeed(seed, seedName); end
end)
AddEventHandler('pw_weed:UseItem', function(item, seedType, data)
    if MFD.iR then MFD:UseItem(item, seedType, data); end
end)
AddEventHandler('pw_weed:SyncPlant', function(plant, del)
    if MFD.iR then MFD:SyncHandler(plant, del); end
end)
AddEventHandler('pw_weed:UseBag',
                function(canUse, msg) MFD:UseBag(canUse, msg); end)

RegisterNetEvent('pw_coke:client:showing')
AddEventHandler('pw_coke:client:showing', function(status) showing = status end)

exports('getCokeShowing', function() return showing end)
