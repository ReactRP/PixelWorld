...
local elevator2BaseX = 918.9307
local elevator2BaseY = -1050.437
local elevator2BaseZ = 31.83833
...
Citizen.CreateThread(function()
    while true do
		...
		elseif (Vdist(912.25, -1055.11, 32.84, pos.x, pos.y, pos.z - 1) < 1.5) then -- Si on est a moins de 1.5 de distance de cette coordonnée
			if IsControlJustReleased(1, Config.Open_key) then -- Si on appuis sur E
				garage_menu = not garage_menu -- on s'assure que le menu soit fermé
	    ...
	    if not la_nacelle_estelle_la and (Vdist(918.9307, -1050.437, 31.83833, pos.x, pos.y, pos.z - 1) < Config.spawndistance) then
